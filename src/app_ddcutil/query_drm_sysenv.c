...
void probe_one_device_using_libdrm(char * devname, int depth) {
   int d1 = depth+1;
   int d2 = depth+2;
   int d3 = depth+3;
   bool debug = false;
   ...
   int dri_fd  = open(devname,O_RDWR | O_CLOEXEC);
   if (dri_fd < 0) {
   ...
   rc = drmGetDevice(dri_fd, &ddev);
   if (rc < 0) {
      rpt_vstring(depth, "Error calling drmGetDevice, errno=%s", linux_errno_desc(errno));
   }
   else {
      rpt_vstring(d1, "bustype:  %d - %s", ddev->bustype, drm_bus_type_name(ddev->bustype));
#ifdef OLD
      rpt_vstring(d1, "bus,dev,domain,func: %d, %d, %d, %d",
            ddev->businfo.pci->bus,
            ddev->businfo.pci->dev,
            ddev->businfo.pci->domain,
            ddev->businfo.pci->func);
#endif
      rpt_vstring(d1, "domain:bus:device.func: %04x:%02x:%02x.%d",
            ddev->businfo.pci->domain,
            ddev->businfo.pci->bus,
            ddev->businfo.pci->dev,
            ddev->businfo.pci->func);
      rpt_vstring(d1, "vendor vid:pid:  0x%04x:0x%04x",
            ddev->deviceinfo.pci->vendor_id,
            ddev->deviceinfo.pci->device_id);
      rpt_vstring(d1, "subvendor vid:pid: 0x%04x:0x%04x",
            ddev->deviceinfo.pci->subvendor_id,
            ddev->deviceinfo.pci->subdevice_id);
      rpt_vstring(d1, "revision id: 0x%04x",
   ...
   drmModeResPtr res = drmModeGetResources(dri_fd);
   if (!res) {
      int errsv = errno;
   ...
   for (int prop_id = 0; prop_id < 200; prop_id++) {
      drmModePropertyPtr prop_ptr = drmModeGetProperty(dri_fd, prop_id);
      if (prop_ptr) {
         if (debug)
   ...
   for (int i = 0; i < res->count_connectors; ++i) {
      ...
      if (!conn) {
         rpt_vstring(d1, "Cannot retrieve DRM connector id errno=%s",
                         res->connectors[i], linux_errno_desc(errno));
         continue;
      }
      if (debug)
         report_drmModeConnector(dri_fd, conn, d1) ;

      // show:
      //   connector_id
      //   connector_type
      //   connector_type_id     - what is this?

      //   connection (i.e is connected?)
      //   EDID property
      //   subconnector properites
      //   summarize remaining properties?

      rpt_vstring(d1, "connector id: %d",       "connector_id:", conn->connector_id);
      ...
      for (int ndx = 0; ndx < conn->count_props; ndx++) {
         if (conn->props[ndx] == edid_prop_id) {
            ...
            drmModePropertyBlobPtr blob_ptr = drmModeGetPropertyBlob(dri_fd, blob_id);
            if (!blob_ptr) {
            ...
            drmModePropertyPtr prop_ptr = drmModeGetProperty(dri_fd, conn->props[ndx]);
            if (prop_ptr) {
               // now in connector report function
               // report_property_value(dri_fd, prop_ptr, conn->prop_values[ndx], 1);
               // to implement:
               summarize_property_value(dri_fd, prop_ptr, conn->prop_values[ndx], d2);
               drmModeFreeProperty(prop_ptr);
            }
            else {
...
/* Filter to find driN files for scandir() */
