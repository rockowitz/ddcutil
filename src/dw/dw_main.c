...
dw_start_watch_displays(DDCA_Display_Event_Class event_classes) {
   ...
   skip_settling_pauses = i2c_all_relevant_buses_rw_by_inode();
   DUAL_MSGNV(debug, DDCA_SYSLOG_NOTICE, "skip_settling_pauses = %s",
                                        sbool(skip_settling_pauses));
   // DECORATED_SYSLOG(DDCA_SYSLOG_NOTICE, "skip_settling_pauses = %s",
   //                                      sbool(skip_settling_pauses));
