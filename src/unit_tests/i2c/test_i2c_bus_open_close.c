/** @file test_i2c_bus_open_close.c
 *
 *  Standalone unit tests for src/i2c/i2c_bus_open_close.c, restricted to
 *  behavior that does not require a real /dev/i2c device: the open functions'
 *  handling of a bus number that cannot exist (-ENOENT, and no EACCES retry
 *  ladder, since ENOENT is not EACCES), and the close functions' handling of
 *  an already-invalid file descriptor (-EBADF).
 *
 *  What is deliberately not exercised: the EACCES retry episode, which needs a
 *  device present but unreadable, and i2c_include_open_failures_reported(),
 *  which writes a file-scope bitset with no accessor to read it back.
 *
