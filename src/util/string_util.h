...
int exactly_matches_anyv(const char * word, ...);
// Like exactly_matches_anyv(), but the caller does not supply a
// terminating NULL; it is appended automatically.
#define EXACTLY_MATCHES_ANYV(word, ...) \
   exactly_matches_anyv(word, ##__VA_ARGS__, NULL)
